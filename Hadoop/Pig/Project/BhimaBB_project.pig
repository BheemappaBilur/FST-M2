...
inputFile1 = LOAD 'hdfs:///user/BhimaBB/inputs/episode1_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile2 = LOAD 'hdfs:///user/BhimaBB/inputs/episode2_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile3 = LOAD 'hdfs:///user/BhimaBB/inputs/episode3_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
...
STORE ordered INTO 'hdfs:///user/BhimaBB/output' USING PigStorage('\t');
